            ...
            <form id="teamCompleteForm" action="{{ route('projects.storeTeam') }}" method="POST" class="space-y-4">
                ...
                <div class="bg-white px-8  rounded-2xl p-5 shadow-md">
                    <input type="text" name="tasks[0][name]" placeholder="Project Title (Required)"
                        class="w-full text-[32px] text-large border-b border-cyan-800 pb-2 focus:outline-none focus:border-cyan-800 placeholder-slate-400"
                        id="teamTasks">
                    <textarea id="text-desc" name="project[0][description]" placeholder="Add Project Description..."
                        class="w-full mt-2 border-b border-cyan-800 focus:outline-none text-medium  focus:border-cyan-800 placeholder:text-normal placeholder:text-sm placeholder:text-md placeholder:text-slate-400 scrollbar-thin"
                        style="max-height: 200px; resize: none;" oninput="autoExpand(this)"></textarea>
                </div>
                <div class="bg-white rounded-2xl p-4 task-item space-y-2">
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="tasks[0][name]"
                                class="border-b p-2 text-medium error-prone {{ $errors->has('tasks.0.name') ? 'border-red-500' : 'border-cyan-800' }} placeholder:text-normal placeholder:text-sm placeholder:text-slate-400 focus:outline-none w-full"
                        ...
                        <!-- Assignee -->
                        <div class="relative">
                            <label for="tasks[0][assignee]" class="text-[12px] text-cyan-700 text-medium">Assignee<span
                                    class="text-red-400"> *</span></label>
                            <input type="text" name="tasks[0][assignee]" id="assignee-input"
                                class="p-2 border-b text-medium text-slate-400 assignee-input error-prone placeholder:text-normal {{ $errors->has('tasks.0.name') ? 'border-red-500' : 'border-cyan-800' }} placeholder:text-sm placeholder:text-slate-400 focus:outline-none w-full pr-10"
                                placeholder="Who will take ownership of this task?" readonly>

                            <!-- Add Member Button -->
                            <button type="button" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-cyan-800"
                    ...
                    <div>
                        ...
                        <textarea name="tasks[0][description]"
                            class="w-full mt-2 border-b {{ $errors->has('tasks.0.name') ? 'border-red-500' : 'border-cyan-800' }} text-medium  resize-none focus:border-cyan-800 placeholder:text-normal placeholder:text-sm placeholder:text-md placeholder:text-slate-400 focus:outline-none scrollbar-thin"
                ...
                <button type="button" id="addTaskBtn"
                    class="w-full p-4 bg-cyan-800 text-white text-medium text-[16px] rounded-xl hover:bg-cyan-900 transition-all flex items-center justify-center gap-2">
                    <i class="fas fa-plus-circle"></i>
                    Add Another Task
                </button>
                <div class="text-right">
                    <button type="submit"
                        class="w-full p-4 bg-cyan-950 text-white text-medium text-[16px] rounded-xl hover:bg-cyan-900 transition-all flex items-center justify-center gap-2 cursor-not-allowed"
                        disabled>
                        Create Project
                    </button>
                </div>
...
<script>
    ...
    function adjustDropdownPosition(buttonId, dropdownId) {
        ...
        if (spaceBelow < dropdownHeight) {
            dropdown.style.top = `-${dropdownHeight + 10}px`; // Display above the button
        } else {
            dropdown.style.top = `${buttonRect.bottom + 10}px`; // Display below the button
        }
    }
    ...
    function toggleDropdown(dropdownId) {
        ...
        if (!dropdown.classList.contains('hidden')) {
            // Adjust position based on space available
            const buttonId = dropdownId === 'due-date-dropdown' ? 'due-date-button' : 'priority-button';
    ...
    function saveDueDate() {
        ...
        if (date && time) {
            alert(`Due Date set to: ${date} ${time}`);
        } else {
    ...
    function addChecklistItem() {
        const points = prompt('Enter the points for this task:');
        if (points !== null && points !== "" && !isNaN(points)) {
            alert(`Points set to: ${points}`);
        } else {
